...
|---|---|---|---|
| `url` | string | Yes | The URL to monitor. Not required for `heartbeat` checks — exit1 generates a unique ping URL automatically. |
| `name` | string | No | Display name (max 200 characters, defaults to URL) |
| `type` | string | No | Check type: `website`, `rest_endpoint`, `redirect`, `ping`, `tcp`, `udp`, `dns`, `websocket`, `heartbeat`, `domain` (default: `website`). Use `domain` to track only domain-expiry (no uptime probe) — pass a bare domain (e.g. `example.com`) as `url`. Requires Nano+ tier. |
