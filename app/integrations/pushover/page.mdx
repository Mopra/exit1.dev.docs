...
Each check has a severity slider under **Alert behavior** in its settings, from **P1 (Critical)** to **P5 (Minimal)**. When a check's severity is anything other than **P3**, it *overrides* the integration's Default priority for that check:

| Check severity | Critical events | Non-critical events |
|---|---|---|
| **P1 — Critical** | **Emergency (2)** | High (1) |
| **P2 — High (default for new checks)** | High (1) | High (1) |
| P3 — Normal | follows the *Default priority* table above | follows the table above |
| P4 — Low | Low (-1) | Low (-1) |
...
New checks default to **P2**, so outages reliably alert at **High** out of the box; bump the truly critical ones to **P1** for Emergency paging or drop noisy ones to P4/P5. **P3** is the neutral setting that defers to the *Default priority* mapping above — it behaves exactly as if severity were never set, so checks created before per-check severity existed are unchanged.
