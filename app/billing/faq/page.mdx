...
You cannot create new monitors beyond your plan limit (50 on Free, 100 on Indie, 250 on Nano, 1,000 on Pro). Existing checks continue to run. Upgrade to the next tier for more capacity.
...
Yes. Every paid plan starts with a 7-day free trial. Cancel before the trial ends
and you are not charged. The trial is available once per account, so switching
plans on an existing subscription starts billing straight away.

The Free plan is separate from the trial. It never expires and needs no card, so
you can also just stay on it and upgrade when you need more capacity.

## Monitoring

### How accurate are the uptime calculations?
Uptime is calculated based on every check execution. Higher tiers check more frequently — Free runs every 5 minutes (288 data points/day), Indie every minute (1,440/day), Nano every 30 seconds (2,880/day), Pro every 15 seconds (5,760/day).
...
MCP is available on **every plan, including Free**. MCP access follows API access — they are the same entitlement, so any plan that can mint an API key can use MCP. Key counts: 1 on Free, 3 on Indie, 10 on Nano, 100 on Pro.
...
Navigate to **Settings** > **API** and click **Create API Key**. Every plan can mint keys — 1 on Free, 3 on Indie, 10 on Nano, 100 on Pro.
